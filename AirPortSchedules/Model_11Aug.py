...
USE_PAPER_HRS_CHECK = True  # Use check (PAPER) for maintenances as in paper
...
Dcheck = {0: 5000.0, 1: 6000.0, 2: 12000.0, 3: 10023.0, 4: 14593.0, 5: 20.0, 6: 14595.0, 7: 10020.0, 8: 10020.0, 9: 10020.0}
                            ...
                            for j in model.P:
                                ...
                                for d in model.D:
                                    if d < FlightData[i]["day_arrival"]:
                                        ...
                                        # model.z_check.add(Mbig * (1-model.z[i, j, d, check]) >= sum(model.fd[a_start, j, d1] for d1 in range(1,d)))
                                        model.z_check.add(model.z[i, j, d, check] == 0)
                                        # model.z_check.add(Mbig * (1-model.z[i, j, d, check]) >= sum(
                                        #     model.x[i,j] for i in F_d_next(0,d) if a_start == FlightData[i]["origin"]))
                        ...
                        for j in model.P:
                            for d in model.D:
                                if d > FlightData[i]["day_arrival"]:
                                    ...
                                    model.z_check2.add(sum(model.z[i, j, d, c] for c in All_Check_List) <=
