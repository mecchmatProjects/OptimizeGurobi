...
$\mathcal W_{jdc}$ denote the candidate flights whose departure time and airport
fall within the duration window associated with an active level-$c$ maintenance
event. The blocking condition is
\begin{equation}
    \gamma_{j,d_i^{\uparrow},c}+x_{ij}\leq1,
    \qquad \forall j\in\Pc,\ c\in\C,\ i\in\mathcal W_{jdc},
    \label{eq:no_flight_during_maintenance}
\end{equation}
which mirrors C15's use of \texttt{m.mega} and absolute departure-time windows.
