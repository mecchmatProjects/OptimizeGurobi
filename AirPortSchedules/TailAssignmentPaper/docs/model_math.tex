...
%%==========================================================================
\section{Maintenance-Extended Model (Constraints C8--C14b)}
%%==========================================================================
...
\begin{equation}
  ...
  \label{eq:c8}
\end{equation}
\textit{In the implementation, C8 is generated from each selected
$(i,j,d,c)$ trigger tuple: same-day rows block departures in the actual
post-arrival duration window, and deferred rows prevent departure from the
maintenance airport while the aircraft waits and on the selected check day.}
...
\begin{equation}
  \sum_{r = d}^{d'} \gamma_{jrc} \geq 1,
  \quad \forall j \in \P,\; c \in \{C,D\},\;
  d,d' \in \D \text{ s.t. } d' - d = d^c_{\max} - 1
  \label{eq:c12}
\end{equation}
\textit{For an explicitly declared horizon $H$, $\D=\{1,\ldots,H\}$; no
post-horizon day is introduced when forming the sliding windows. A/B checks
are controlled by C13/C13b rather than a calendar-spacing row.}

\textit{The trigger-specific C8 maintenance-window constraints are the active
flight-blocking formulation. The earlier C15 calendar-day block and a deferred-
trigger equality block were retired because they could constrain flights or
maintenance starts outside the selected trigger interval.}
