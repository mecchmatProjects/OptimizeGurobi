...
\begin{enumerate}
\item \textbf{Basic feasibility correction (C1--C4 / Khaled constraints (3)--(6)).}
...
\item \textbf{Cumulative-flight-time row correction.}  We show that the cumulative-flight-
  time maintenance constraint (constraint (13) in \citet{khaled2018compact})
  contains a logical flaw in opening and one-endpoint intervals, and in the handling of consecutive flights and maintenance events. 
...
\item \textbf{Event-based reformulation.}  The day-indexed type-A model
  ...
  C1--C13 requirements, in which each flight's own timestamp supplies all
  ...
  the legacy cumulative-flight-time row.
\end{enumerate}

\subsection*{Paper organisation}

\Cref{sec:literature} reviews related work.  \Cref{sec:basic} restates the
basic TAP model and proves a C1--C4 corner-case correction closed by C1--C5.  \Cref{sec:maintenance}
presents the maintenance-extended model, the cumulative-flight-time row correction, and
the induced interpretation updates for C8--C13 under the corrected basic model.
...
\Cref{sec:computational} reports the numeric cumulative-flight-time counterexample and the four-method benchmarking results, and
