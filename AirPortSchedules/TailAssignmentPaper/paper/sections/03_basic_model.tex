...
Let
\paragraph{C0 — Binary assignment domain.}
...
\paragraph{Corrected C0--C5 feasibility criterion.}
...
\paragraph{C4 — Pairwise non-overlap.}
\begin{equation}\label{eq:c4_overlap}
  x_{i_1 j} + x_{i_2 j} \le 1,
  \qquad \forall \{i_1,i_2\}\in\mathcal{O},\; \forall j\in\Pc.
\end{equation}

Let $\mathcal{K}_j$ denote the maximal cliques of the interval-overlap graph
for aircraft $j$. The clique strengthening is:
\paragraph{C5 — Clique-strengthened non-overlap.}
\begin{equation}\label{eq:c5}
  \sum_{i\in K}x_{ij}\leq 1,
  \qquad \forall j\in\Pc,\; K\in\mathcal{K}_j.
\end{equation}

The corrected core model is therefore
\eqref{eq:c0}--\eqref{eq:c5}.
