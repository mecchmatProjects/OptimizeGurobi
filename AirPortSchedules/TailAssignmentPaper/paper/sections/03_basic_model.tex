...
\begin{equation}\label{eq:c4_overlap1}
  x_{i_1 j} + x_{i_2 j} \le 1,
  \qquad \forall \{i_2\}\in\mathcal{O_{i_1}},\; \forall j\in\Pc. 
\end{equation}

However, the better way to enforce non-overlap is to consider cliques of mutually overlapping flights, rather than just pairwise constraints.
Define a clique as a set of flights that are pairwise overlapping. 
Let $\mathcal{C_i}$ be the set of all maximal cliques in the conflict graph defined by $\mathcal{O_i}$, i.e.,
\begin{equation}
  \mathcal{C_i}=\{C\subseteq\F: i' \in \mathcal{O_i} \text{ for all } i'\in C\}. 
\end{equation}
For each clique, impose the following constraint:
\paragraph{C5 — Pairwise non-overlap clique.}
\begin{equation}\label{eq:c5_overlap}
  \sum_{i\in C} x_{i j} \le 1,
  \qquad \forall C\in\mathcal{C_i},\; \forall j\in\Pc.
\end{equation}



The corrected core model is therefore
\eqref{eq:c0}--\eqref{eq:c5_overlap}.
...
\begin{proposition}[Proposition~1]
  Any feasible solution to \eqref{eq:c0}--\eqref{eq:c5_overlap}
