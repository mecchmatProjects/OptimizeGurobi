...
\begin{equation}\label{eq:c11_paper}
...
\subsection{Constraint~C(11): original formulation and its flaw}
\label{subsec:c11_flaw}

The original paper's cumulative-flight-time constraint is:
\begin{equation}\label{eq:c11_paper}
  \tag{C11$_{\mathrm{paper}}$}
  \sum_{i \in F_{d,d'}} x_{ij}\,\tilde{t}_i
  \;\le\;
  T^A_{\max}
  + M_{dd'}\Bigl(2 - y_{jd} - y_{jd'}\Bigr)
...
\begin{proof}
  ...
  into~\eqref{eq:c11_paper} gives the stated RHS.  Since $M_{dd'}$ is chosen
...
\eqref{eq:c11_paper} consists of two constraints, each relaxed by one endpoint:

\begin{equation}\label{eq:c11_new_a}
  \tag{C11a}
  \sum_{i \in F_{d,d'}} x_{ij}\,\tilde{t}_i
  \;\le\;
  T^A_{\max}
  ...
  + M_{dd'}\!\sum_{r \in [d+1,d'-1]} y_{jr},
\end{equation}
\begin{equation}\label{eq:c11_new_b}
  \tag{C11b}
  \sum_{i \in F_{d,d'}} x_{ij}\,\tilde{t}_i
  \;\le\;
  T^A_{\max}
...
\begin{lemma}[The endpoint split is not sufficient]
  ...
  constraints~\eqref{eq:c11_new_a}--\eqref{eq:c11_new_b} do not necessarily
...
\begin{remark}[Implementation status]
  ...
  inequalities~\eqref{eq:c11_new_a}--\eqref{eq:c11_new_b}; the original
  single-row form~\eqref{eq:c11_paper} remains available as an explicit,
