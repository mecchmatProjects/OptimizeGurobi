%%==========================================================================
\section{The Maintenance-Extended Model and Correction of Constraint~(11)}
...
\paragraph{C5 — Maintenance blocks same-day subsequent flights.}
...
For the type-A-only model studied in this paper, C9 is not an additional
calendar-spacing requirement: type-A maintenance is governed by the exact
flight-hour state in C13. The displayed row is retained as the generic
...
\paragraph{C10 — At most one maintenance event per aircraft per day.}
\begin{equation}\label{eq:c10}
  \sum_{m\in\MA}\sum_{i \in F_d^{\downarrow}(m)} z_{ijd} \le 1,
  \qquad \forall j \in \Pc,\; d \in \D.
\end{equation}

For binary $y$, C10 is implied by C8; it is retained because it makes the
...
\subsection{Impact of corrected C2--C4 feasibility on C5--C10}
...
\paragraph{C5.}
No algebraic change is required in~\eqref{eq:c5}; however, with
...
\paragraph{C6--C10.}
Constraints~\eqref{eq:c6}--\eqref{eq:c10} remain structurally unchanged. C7 is inactive in the type-A-only model. Their
feasible region is tightened indirectly because $z$ and $y$ are linked to a
strictly smaller, physically valid $x$-space.

\paragraph{C9 and onward.}
...
\subsection{Rigorous feasibility theorem for C1--C10}
\label{subsec:feasibility_theorem_c1_c10}

We now state the corrected sufficiency result for the type-A maintenance model.
Consider the constraint family
\eqref{eq:c1}--\eqref{eq:c3}, \eqref{eq:c4_overlap}, \eqref{eq:c5}--\eqref{eq:c10},
\eqref{eq:c11_state_lower}--\eqref{eq:c11_state_bound}, and the stated
variable domains.

\begin{theorem}[Physical feasibility under corrected C1--C10]
  \label{thm:physical_feasibility_c1_c10}
...
\begin{proof}
  ...
  assigned flights. By the aggregation equality~\eqref{eq:c8}, each day's
  maintenance trigger is represented by the single aggregate indicator $y_{jd}$;
  by the daily cap~\eqref{eq:c10}, at most one maintenance event is attached to
  aircraft $j$ on day $d$.
...
\subsection{Issues of day-indexed maintenance constraints C1-C10}
...
\item The day-indexed formulation requires a large number of big-$M$ constants
  ...
  \item The original cumulative-flight-time constraint (C11) in the day-indexed formulation is flawed, as it does not correctly enforce the flight-hour limit between two consecutive maintenance nights. This flaw can lead to infeasible solutions that violate the intended maintenance intervals.
\end{itemize}


\subsection{Constraint~(11): original formulation and its flaw}
\label{subsec:c11_flaw}

The original paper's cumulative-flight-time constraint is:
\begin{equation}\label{eq:c11_legacy}
  \sum_{i \in F_{d,d'}} x_{ij}\,\tilde{t}_i
  \;\le\;
  T^A_{\max}
  + M_{dd'}\Bigl(2 - y_{jd} - y_{jd'}\Bigr)
...
\begin{proof}
  ...
  into~\eqref{eq:c11_legacy} gives the stated RHS. Since $M_{dd'}$ is chosen
...
\subsection{Constraint~(11): endpoint-split strengthening and its limitation}
\label{subsec:c11_corrected}

An endpoint-split strengthening considered for replacing
\eqref{eq:c11_legacy} consists of two constraints, each relaxed by one endpoint:
...
\begin{remark}[Implementation status]
  ...
  inequalities~\eqref{eq:c11_split_a}--\eqref{eq:c11_split_b}; the original
  single-row form~\eqref{eq:c11_legacy} remains available as an explicit,
  ...
  The computational study in \Cref{sec:computational} numerically confirms both Lemma~\ref{lem:c11_flaw}
  and Lemma~\ref{lem:c11_split_limit} against the real built constraint
...
\eqref{eq:c11_state_lower}--\eqref{eq:c11_state_bound}, the same data give
...
\subsection{Constraint~(11): exact cumulative-state formulation}
