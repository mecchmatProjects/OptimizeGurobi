...
Constraints~\eqref{eq:c1}--\eqref{eq:c5} are retained.  Additional
constraints:
...
\subsection{Impact of corrected C1--C5 feasibility on C8--C13}
...
In summary, the C1--C5 correction primarily changes the domain over which the
...
\subsection{Rigorous feasibility theorem for C1--C13}
\label{subsec:feasibility_theorem_c1_c13}

We now state the corrected sufficiency result for the type-A maintenance model.
Consider the constraint family
\eqref{eq:c1}--\eqref{eq:c5}, \eqref{eq:c8}--\eqref{eq:c11}, \eqref{eq:c13},
...
\begin{theorem}[Physical feasibility under corrected C1--C13]
  \label{thm:physical_feasibility_c1_c13}
...
$h_{j4}\leq T^A_{\max}=10$. The day-indexed exact-state rows below implement
this recursion; the ordered prefix-state model of
\Cref{sec:event_reformulation} implements the same hour-limit along the
selected route without day-pair windows. The comparison harness
\texttt{experiments/compare\_formulations.py} selects the legacy and
exact-state builders explicitly.

\begin{table}[t]
\centering\small
\caption{Day-level data for the four-day counterexample.}
\label{tab:c11_counterexample_day}
\begin{tabular}{ccccc}
\toprule
Day $d$ & $y_{jd}$ & $v_{jd}$ & Exact state $h_{jd}$ & Check of $h_{jd}\le T^A_{\max}$ \\
\midrule
1 & 1 & 0  & 0  & holds \\
2 & 0 & 10 & 10 & holds \\
3 & 0 & 0  & 10 & holds \\
...
\begin{table}[t]
\centering\small
\caption{Window-level comparison for the same counterexample ($T^A_{\max}=10$, $M_{dd'}=20$).}
\label{tab:c11_counterexample_windows}
\begin{tabular}{lcccc}
\toprule
Window $(d,d']$ & LHS $\sum_{i\in F_{d,d'}} x_{ij}\tilde t_i$ & Split-A RHS & Split-B RHS & Paper-row RHS \\
\midrule
$(1,3]$ & 10 & 30 & 10 & 30 \\
$(2,4]$ & 10 & 10 & 30 & 30 \\
$(1,4]$ & 20 & 30 & 30 & 10 \\
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/c11_counterexample_timeline.png}
\caption{Timeline view of the four-day counterexample: bars show daily flying time $v_{jd}$, markers show exact cumulative state $h_{jd}$, the dashed line is $T^A_{\max}=10$, and dotted vertical lines mark maintenance nights ($d=1,4$).}
\label{fig:c11_counterexample_timeline}
\end{figure}

Tables~\ref{tab:c11_counterexample_day} and~\ref{tab:c11_counterexample_windows}
show why the endpoint split can accept the infeasible pattern: the long window
$(1,4]$ has LHS 20 while both split rows remain relaxed at 30. In contrast,
the exact state trajectory in \Cref{fig:c11_counterexample_timeline} reaches
$h_{j4}=20>T^A_{\max}=10$, so the violation is detected directly.
...
As we see, the fixes to the original paper's maintenance model are twofold: (i) the C1--C5 correction, which ensures that the flight assignment is physically feasible, and (ii) the cumulative-flight-time row correction.
