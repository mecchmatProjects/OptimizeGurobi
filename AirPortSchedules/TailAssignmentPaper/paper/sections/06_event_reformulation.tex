...
C2--C5: flight coverage, non-home continuity, home-airport
continuity/turn-time balance, and pairwise non-overlap remains active on the
...
The maintenance blocking and capacity constraints are formulated as follows. 
We assume that every maintenance event starts immediately after the triggering flight. 
In this case we can rewrite constraints C8 in the next way:

\paragraph{E14. Maintenance blocking.}
Define $F^{\text{dep}}_{ra}$ as the set of flights from airport $a$ whose departure falls before the event's completion plus turn time:
\begin{equation}
	F^{\text{dep}}_{ra} = \{ i \in \F \mid \alpha(i) = a,\ t_i^{\text{dep}} < t_{r}^{\text{arr}} + T_F \}.
\end{equation}
If $z_{rj}=1$, every selected flight whose
departure falls before the event's completion plus turn time is blocked:
\begin{equation}
x_{ij}+z_{rj}\leq 1,
\qquad \forall i \in F^{\text{dep}}_{ra},\ j \in \Pc.
\end{equation}
Only economically relevant timestamp conflicts are generated.


\paragraph{E15. Maintenance capacity.}
Define $F^{\text{arr}}_{ra}$ as the set of flights to airport $a$ whose arrival 
falls before the event's completion plus turn time:
\begin{equation}
	F^{\text{arr}}_{ra} = \{ i \in \F \mid \beta(i) = a,\ t_i^{\text{arr}} < t_{r}^{\text{arr}} + T_F \}.
\end{equation}
At every airport timestamp breakpoint, the active maintenance events satisfy
\begin{equation}
\sum_{(r,j)\in F^{\text{arr}}_{ra}\times \Pc} z_{rj}
\leq \operatorname{mcap}_{a},
\label{eq:ordered_c10_capacity}
\end{equation}
This is implemented by the timestamp-overlap component
\texttt{c10\_event\_capacity}.
