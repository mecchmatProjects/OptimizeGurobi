...
\qquad \forall \ell\in\F,\ j\in\Pc,\ \tD{\ell}>\tI{i_r}.
\end{equation}
...
M(x_{\ell j}-1)+M(z_{rj}-1) + \tD{\ell} \ge \tI{i_r}+s_{rj}+\Delta t,
\qquad \forall \ell\in\F,\ j\in\Pc,\ \tD{\ell}>\tI{i_r}.
\end{equation}
...
T_{rj} - \delta_A < T_{ij'} < T_{rj}+\delta_A\right\},
\end{equation}
that is, the events at airport $a$ whose maintenance windows~\eqref{eq:E16_flex_window} intersect
that of $(r,j)$. Occupancy is then limited by
\begin{equation}\label{eq:E15_flex}
\sum_{(i,j')\in\mathcal{Q}(a,r,j)}z_{i j'}
\leq \operatorname{mcap}_{a},
\qquad \forall a\in \MA,\ r\in \mathcal{R},\ j\in\Pc .
\end{equation}
Because $\mathcal{Q}(a,r,j)$ depends on the decision variables, \eqref{eq:E15_flex} is linearised with
one overlap indicator and one ordering indicator per potentially conflicting event pair; the
construction is given in Section~\ref{subsec:flex_complexity}.
...
\delta_A\,z_{rj}\ \le\ s_{rj}\ \le\ (\delta_A+W)\,z_{rj},
...
weakens the linear relaxation. Second, \eqref{eq:E15_flex} is linearised with one overlap
indicator and one ordering indicator per potentially conflicting flight pair at a maintenance
station, together with two disjunctive separation rows per pair and one capacity row per
flight. Writing $\mathcal{K}_a$ for the set of such pairs at airport $a$, the capacity family
is $O\bigl(\sum_a|\mathcal{K}_a|\bigr)$ rows and the same order of binary variables, against
...
With $W$ equal to one day, the measured cost on the matched grids of \Cref{sec:computational}
is an increase of roughly one fifth in variables and one half in rows relative to the
immediate-start implementation.

Finally, the variant is a relaxation. Fixing $s_{rj}=\delta_A z_{rj}$ satisfies
\eqref{eq:E16_flex_s_basic}, reduces \eqref{eq:E14_flex} to \eqref{eq:E14_rewritten} and
reduces \eqref{eq:E15_flex} to \eqref{eq:E15_not_full}; hence every solution of the
immediate-start model is feasible for the flexible one, and its optimal value can only
improve. The additional freedom is not free: an aircraft that defers a check is grounded
