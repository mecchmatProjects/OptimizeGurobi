...
we introduce another variable representing the flexible end time of maintenance events. 
...
The new variable can be denoted as $s_{rj}$, representing the start time of the maintenance event for flight $r$ and aircraft $j$.

In this case the constraints \ref{eq:E14} and \ref{eq:E15_not_full} turn into the next ones:

\begin{equation}\label{eq:E14_flex}
M(x_{r'j}-1) + \tD{j} \ge \tI{r} z_{rj}+s_{rj}+\Delta t, \quad \forall (r',j)\in\F\times\Pc, \ r'>r.
\end{equation}

This constraint ensures that the start time of a maintenance event for a given flight and aircraft combination is respected, 
...
Define the modified event set
\begin{equation}
\mathcal{Q}(a,r)=\left\{(i,j):\aI{i}=a,\quad
s_{rj} - \delta_A \leq \tD{i} < s_{rj}\right\}.
\end{equation}

\begin{equation}\label{eq:E15_flex}
\sum_{(i,j)\in\mathcal{Q}(a,r)}z_{i j}
\leq \operatorname{mcap}_{a},
\qquad \forall a\in \MA,\ r\in \mathcal{R}.
\label{eq:ordered_c10_capacity}
\end{equation}

We also need the basic condition on $s_{rj}$:
\begin{equation}\label{eq:E16_flex_s_basic}
s_{rj} \ge \tD{r} + \Delta t, \quad \forall r\in \mathcal{R},\ j\in \Pc.
\end{equation}
