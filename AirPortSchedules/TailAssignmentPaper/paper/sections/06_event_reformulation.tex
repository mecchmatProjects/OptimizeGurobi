%%==========================================================================
\section{Ordered Event-Based Reformulation for a Flight-hour Maintenance}
\label{sec:event_reformulation}
%%==========================================================================

In this paper, we present an ordered event-based model for
flight-hour maintenance. It removes the day index and does not introduce the
standalone successor-arc event formulation. Flights are processed in
nondecreasing departure-time order, and maintenance is represented by a binary
event attached to the flight that triggers it. Thus the design-note variables
$x_{ij}$ and $z_{ij}$ are represented below through an ordered position index
$r$ that maps to a flight index $i_r$.

\subsection{Ordered event representation}

For flight $i$, let $o_i$ and $a_i$ be its departure and arrival airports,
$p_i$ its flying time, and $\tD{i}$ and $\tI{i}$ its departure and arrival
timestamps (the same macros as in \Cref{sec:basic}). For aircraft $j$, let $a^0_j$ be its initial airport.

Define the ordered-position index set
\[
\mathcal{R}=\{1,\ldots,n_f\},
\]
and let $(i_1,\ldots,i_{n_f})$ be flights sorted by $(\tD{i},i)$. Position
$r\in\mathcal{R}$ refers to flight $i_r$. This is the only role of $r$: it is
an order index over flights. Define
\[
\mathcal{R}^M=\{r\in\mathcal{R}: i_r\in\F^M\},
\]
and use the following variables:
\paragraph{E1-E3. Ordered event variables.}
\begin{itemize}
\item $x_{rj}\in\{0,1\}$: aircraft $j$ operates flight $i_r$;
\item $z_{rj}\in\{0,1\}$: a flight-hour maintenance event is performed after flight $i_r$ on aircraft $j$;
\item $h_{rj}\ge0$: cumulative flight time of aircraft $j$ immediately after processing position $r$.
\end{itemize}
Because $r\leftrightarrow i_r$ is one-to-one, the relation to the original
notation is direct: $x_{rj}=x_{i_r j}$ and $z_{rj}=z_{i_r j}$.

A maintenance event after flight $i_r$ is assignment-linked, starts at
The corrected airport-continuity, turn-time, and overlap constraints remain
active on the assignment variables. In particular, an assignment is allowed
only when it can be connected to the aircraft's preceding assigned flight and
when the required maintenance duration leaves enough time before the next
assigned departure.

The objective is
\begin{equation}
\min\ \sum_{r\in\mathcal{R}}\sum_{j\in\Pc}c_{i_rj}x_{rj}
 +\sum_{r\in\mathcal{R}^M}\sum_{j\in\Pc}mc_{i_r j}\,z_{rj}.
\label{eq:ordered_event_objective}
\end{equation}

The constraints for $x_{rj}$ time-turn retain from the previous formulations, i.e.constrants E4--E7 the same as C2--C5 in our corrected formulation.

\subsection{Prefix-state propagation}

Let $T_F:=T^A_{\max}$ be the maximum flying time between flight-hour maintenance events and let $\Phi_j$
be the pre-horizon accumulated flying time of aircraft $j$. At every ordered
position, an unassigned flight carries the state forward, an assigned flight
adds its duration, and an event resets the state after its triggering flight.
Using valid position-specific big-$M$ values $M_{rj}$, the transition is
represented by
\paragraph{E8--E14. Prefix-state propagation.}
\begin{align}
h_{rj}&\ge h_{r-1,j}-M_{rj}x_{rj},\\
h_{rj}&\le h_{r-1,j}+M_{rj}x_{rj},\\
h_{rj}&\ge h_{r-1,j}+p_{i_r}-M_{rj}(1-x_{rj})-M_{rj}z_{rj},\\
h_{rj}&\le h_{r-1,j}+p_{i_r}+M_{rj}(1-x_{rj})+M_{rj}z_{rj},\\
h_{rj}&\ge 0,\\
h_{rj}&\le T_F(x_{rj}-z_{rj})+M_{rj}(1-x_{rj}),\\
h_{rj}&\le T_F,\\
h_{r-1,j}+p_{i_r}&\le T_F+M_{rj}(1-x_{rj}),
\label{eq:ordered_hour_transition}
\end{align}
The first two rows carry the state at an unassigned position. The next two
rows impose accumulation for an assigned flight unless an event is selected.
The fifth and sixth rows together give the exact post-event state:
$h_{rj}=0$ when $z_{rj}=1$, because the maintenance occurs after flight
$i_r$. The seventh row bounds the state in all cases. The final row prevents a maintenance event
from repairing an arrival that already exceeds $T_F$.
The initial state is $h_{0j}=\Phi_j$.

 \paragraph{E15. Event assignment and compatibility.}
Events satisfy
\begin{equation}
z_{rj}\le x_{rj},
\qquad \forall r\in\mathcal{R}^M,\ \forall j\in\Pc,
\label{eq:ordered_event_assignment}
\end{equation}
and timestamp-based incompatibility rows prevent an event from being
followed by a departure that occurs before the maintenance duration and turn
time have elapsed. These rows preserve physical route feasibility while
avoiding a separate successor-variable family. Capacity is enforced directly
by event-overlap inequalities at each airport and timestamp breakpoint. This
is equivalent to maintaining an airport occupancy state $u_{a,\theta}$ and
imposing $u_{a,\theta}\leq\mathrm{mcap}_{a,\theta}$, but it requires no
additional occupancy variable.

\paragraph{E16. Event capacity.}
The ordered event rows correspond to the day-indexed maintenance constraints:
event assignment is C9, duration blocking is C8, and timestamp-overlap
capacity is C10. The at-most-one-event-per-aircraft-per-day rule is retained
explicitly as
\begin{equation}
\sum_{r\in\mathcal{R}:\,\lfloor\tI{i_r}/1440\rfloor=d}z_{rj}\le1,
\qquad \forall j\in\Pc,\ d\in\D,
\label{eq:ordered_event_one_day}
\end{equation}
which is the event-indexed counterpart of C13. We keep it here in this way in order to keep constraint consistency with the legacy model,
but now we can easily reformulate even by-day events to be consistent with any or every time-zone needed.

\paragraph{E17. Maintenance capacity.}
At every airport timestamp breakpoint, the active maintenance events satisfy
\begin{equation}
\sum_{(r,j)\in\mathcal{I}(a,\theta)}z_{rj}
\leq \operatorname{mcap}_{a,\theta},
\label{eq:ordered_c10_capacity}
\end{equation}
where $\mathcal{I}(a,\theta)$ contains events whose maintenance intervals
cover $\theta$. This is implemented by the timestamp-overlap component
\texttt{c10\_event\_capacity}; it is the maintenance-capacity constraint that was not
explicitly displayed earlier.

\paragraph{E18. Maintenance blocking.}
If $z_{rj}=1$, every selected flight whose
departure falls before the event's completion plus turn time is blocked:
\begin{equation}
x_{\ell j}+z_{rj}\leq1.
\label{eq:ordered_c8_block}
\end{equation}
Only economically relevant timestamp conflicts are generated.


\subsection{Complete constraint inventory of the implemented A-only model}

The following list gives every constraint family constructed by the current
ordered A-only builder. It distinguishes the numbered legacy requirements from
the auxiliary rows introduced by the event reformulation.

\begin{description}
\item[C0--C1: assignment domain and coverage.] The variables $x_{rj}$ are
binary, and each ordered flight is assigned exactly once:
\begin{equation}
\sum_{j\in\Pc}x_{rj}=1,\qquad r\in\mathcal{R}.
\label{eq:ordered_c0_c1}
\end{equation}

\item[C2--C3: aircraft continuity and turn time.] The corrected equipment-flow
balance is applied to $x_{rj}$, including the initial aircraft position and
the minimum turn time. The implementation is
\texttt{\_add\_c23\_turn}.

\item[C4: pairwise non-overlap.] For every temporally conflicting flight pair
and aircraft, the builder adds
\begin{equation}
x_{ij}+x_{\ell j}\leq1.
\label{eq:ordered_c4_pairwise}
\end{equation}
These rows are constructed by \texttt{c4\_pairwise\_overlap}.

\item[C5: clique-strengthened non-overlap.] For every maximal interval clique
$K$ for aircraft $j$, the builder adds
\begin{equation}
\sum_{i\in K}x_{ij}\leq1.
\label{eq:ordered_c5_clique}
\end{equation}
These rows are constructed by \texttt{c5\_clique\_overlap}. The default
ordered A-only mode activates both C4 and C5; this is the literal corrected
basic formulation, although C5 dominates many C4 rows.

\item[C6--C7: event variables.] The maintenance event variables $z_{rj}$ are
binary and are instantiated only for flights arriving at a maintenance-capable
airport. There is no independent daily $y$ variable in this formulation.
An auxiliary event-type exclusivity row limits each triggering flight and
aircraft to at most one check type. Its practical meaning is that one arrival
cannot simultaneously start two different maintenance types.

\item[C8: maintenance blocking.] If $z_{rj}=1$, every selected flight whose
departure falls before the event's completion plus turn time is blocked:
\begin{equation}
x_{\ell j}+z_{rj}\leq1.
\label{eq:ordered_c8_block}
\end{equation}
Only economically relevant timestamp conflicts are generated.

\item[C9: event assignment.] An event requires its triggering flight:
\begin{equation}
z_{rj}\leq x_{rj}.
\label{eq:ordered_c9_assignment}
\end{equation}
The reverse inequality would force maintenance after every eligible flight and
is not the intended maintenance-request semantics.

\item[C10: maintenance capacity.] At every airport timestamp breakpoint, the
active maintenance events satisfy
\begin{equation}
\sum_{(r,j)\in\mathcal{I}(a,\theta)}z_{rj}
\leq \operatorname{mcap}_{a,\theta},
\label{eq:ordered_c10_capacity}
\end{equation}
where $\mathcal{I}(a,\theta)$ contains events whose maintenance intervals
cover $\theta$. This is implemented by the timestamp-overlap component
\texttt{c10\_event\_capacity}; it is the maintenance-capacity constraint that was not
explicitly displayed earlier.

\item[C11: event-to-day aggregation.] The day indicator is eliminated, but it
can be recovered as
\begin{equation}
y_{jd}=\sum_{r:\,\lfloor\tI{i_r}/1440\rfloor=d}z_{rj}.
\label{eq:ordered_c11_recovery}
\end{equation}

\item[C12: calendar spacing.] C12 is inactive in the A-only model. It is
implemented only by the C/D calendar extension, which is not enabled here.

\item[C13: at most one event per aircraft per day.] The explicit daily cap is
\begin{equation}
\sum_{r:\,\lfloor\tI{i_r}/1440\rfloor=d}z_{rj}\leq1.
\label{eq:ordered_c13_day_cap}
\end{equation}

\item[Hour-limit state rows.] The auxiliary prefix state $h_{rj}$ is
nonnegative, carries forward at unassigned positions, accumulates an assigned
flight when $z_{rj}=0$, resets to zero when $z_{rj}=1$, and is bounded by
$T^A_{\max}$. These are the implemented exact A-hour rows in
\texttt{event\_hour\_state}; they replace the legacy day-pair C13 row rather than
adding another numbered C-constraint.
\end{description}

The event formulation therefore has no additional business rule hidden under
an unnumbered C-constraint. The only auxiliary families are the event-type
exclusivity rows just described and the linearization rows for the C13 hour
state. The airport occupancy quantity $q_{a,j}$ proposed in the design note is
not introduced as a variable: its role is represented directly by the C10
timestamp-capacity sums, which is smaller and equivalent for the A-only case.

Table~\ref{tab:ordered_c1_c13} records the correspondence with the numbered
day-indexed requirements. Some legacy rows disappear because their variables
are eliminated; this is a formulation change, not an omitted feasibility
condition.

\begin{table}[t]
\centering\scriptsize
\caption{Coverage of the numbered C0--C13 requirements in the ordered A-only
	formulation.}
\label{tab:ordered_c1_c13}
\begin{tabular}{p{0.10\linewidth} p{0.30\linewidth} p{0.48\linewidth}}
\toprule
Requirement & Day-indexed role & Ordered A-only counterpart \\
\midrule
C0 & Binary assignment domain & Binary domain on $x_{rj}$. \\
C1 & Flight coverage & Coverage equation on $x_{rj}$ (equivalently $x_{i_rj}$). \\
C2--C3 & Airport continuity and turn time & Corrected equipment-flow balance on assignment variables. \\
C4 & Pairwise non-overlap & Pairwise overlap inequalities on $x$. \\
C5 & Clique-strengthened non-overlap & Maximal-clique inequalities on $x$. \\
C6--C7 & Maintenance variables $z,y$ & Event indicator $z_{rj}$; the daily indicator $y$ is not a primitive variable. \\
C8 & Maintenance blocks later flights & Timestamp blocking using $\delta_F+\dt$. \\
C9 & Maintenance requires assignment & The event-assignment inequality above. \\
C10 & Maintenance capacity & Direct timestamp-overlap capacity rows, equivalent to an airport occupancy state. \\
C11 & Link event triggers to a daily indicator & Daily indicator is eliminated; it is recoverable as $y_{jd}=\sum_{r:\,\lfloor\tI{i_r}/1440\rfloor=d}z_{rj}$. \\
C12 & Calendar-time spacing & Inactive for type-A-only maintenance in this paper. \\
C13 & At most one event per aircraft per day & Explicit daily event inequality~\eqref{eq:ordered_event_one_day}. \\
-- & Cumulative flight-hour limit (Khaled~(13)) & Prefix-state recursion~\eqref{eq:ordered_hour_transition} with $h_{rj}\le T^A_{\max}$. \\
\bottomrule
\end{tabular}
\end{table}

\begin{proposition}[Ordered prefix-state validity]
\label{prop:ordered_event_correct}
Assume the corrected route constraints define a physically feasible integral
assignment, every event is attached to an assigned flight at a
maintenance-capable airport, and the supplied initial state is valid. Then the
ordered prefix-state formulation satisfies the type-A flight-hour limit.
\end{proposition}

\begin{proof}
Induct on the ordered flight positions. An unassigned position carries the
state unchanged. An assigned position adds its flying time unless an event is
selected, in which case the two reset rows set the state to the current
flight's duration. The final transition row and the upper state bound prevent
the threshold from being exceeded before a reset. The induction starts from
$\Phi_j$, so every assigned route satisfies the type-A limit.
\end{proof}

\subsection{Why the ordered formulation is the better method}

\subsection{Theoretical complexity and model size}

Let $n_f=|\F|$, $n_p=|\Pc|$, and let $n_m$ denote the number of flights
that arrive at a maintenance-capable airport. In the A-only ordered model,
the assignment variables contribute at most $n_fn_p$ binary variables, while
the maintenance-event variables contribute at most $n_mn_p$ binary variables.
The aircraft-local prefix state contributes at most $n_fn_p$ continuous
variables; with sparse aircraft eligibility, all three counts are smaller and
are determined by the feasible $(i,j)$ pairs. Thus the principal variable
bound is
\[
O(n_fn_p+n_mn_p),
\]
with no additional day-indexed $y_{jd}$ or occupancy-state $q_{a,j}$ family.
The ordered implementation instead stores one state position per feasible
flight--aircraft pair.

The main linear constraint families are $O(n_f)$ for coverage,
$O(n_fn_p)$ for continuity and event assignment, $O(n_fn_p)$ for the
prefix-state equations, and $O(n_pD)$ for the daily event cap. Pairwise
non-overlap and timestamp blocking depend on the conflict sets and are
$O(|\mathcal{O}|n_p)$ and $O(|\mathcal{B}|)$, respectively, where
$\mathcal{O}$ is the flight-overlap set and $\mathcal{B}$ is the generated set
of event--following-flight blocking pairs. Timestamp-capacity rows are
generated per airport breakpoint, so their number is proportional to the
number of distinct event timestamps rather than to a full time grid. In the
worst case, $|\mathcal{O}|=O(n_f^2)$ and the sparse conflict families can also
be quadratic, but timetable sparsity usually makes their observed counts much
smaller. The theoretical conclusion is therefore precise: the Event-based
model removes the day dimension from the principal variable families and
keeps prefix-state size linear in feasible flight--aircraft pairs; it does not
guarantee fewer total constraints unless the induced conflict and blocking
sets are also sparse.

The ordered formulation combines event-triggered maintenance with a global
flight order and aircraft-local prefix states. It removes the day-pair
accumulation ambiguity of the legacy cumulative-flight-time row, avoids successor-variable
expansion, and keeps the physical route constraints needed for valid tail
assignments. For type-A maintenance, C12's calendar-spacing role is inactive:
the flight-hour limit is enforced by the exact prefix-state recursion. The
computational study in
\Cref{sec:computational} evaluates this formulation against the legacy 
model on matched horizon and fleet-size grids.
