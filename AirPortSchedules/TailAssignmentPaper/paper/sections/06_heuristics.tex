...
We study three heuristic approaches. Candidate routes are screened by the
$\{A,B,C,D\}$ counter logic in the timeline simulator described below. A run
that leaves flights unassigned is a partial assignment, not a feasible TAP
solution, and its raw route cost is not directly comparable with a complete
MILP objective.
...
\paragraph{Observed deterministic--stochastic ordering.}
The space-time network has a strict temporal ordering (no back-edges in time).
On the current suite, the deterministic methods cover more flights and run
faster than ACO. This observation does not establish near-optimality of the
modified Dijkstra rule: its lexicographic coverage/cost update differs from
standard shortest-path Dijkstra, and the greedy method has better aggregate
coverage in \Cref{tab:heuristic_comparison}.
...
Only the three complete assignments per deterministic method admit direct
cost comparisons with complete solutions. The averages over all eight rows
primarily measure coverage and runtime; they are not optimality-gap evidence.
