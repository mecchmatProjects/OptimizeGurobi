...
external solver required).  The code, instance generator, and experiment
...
Three methods are compared throughout the computational study:
...
All three methods share the same instance set, cost matrix, and threshold
parameters; the comparison is therefore controlled for data.  Every run
emits the same output schema (see \texttt{experiments/run\_batch.py}):
\texttt{stem}, \texttt{method}, \texttt{status}, \texttt{objective},
\texttt{gap\_pct}, \texttt{cpu\_s}, \texttt{num\_vars}, \texttt{num\_constraints}.
...
both the greedy-insertion heuristic and a repaired variant; the MILP rows are
classified as \texttt{solver\_limit} on the small quick subset because the
local CPLEX Community Edition exceeds the model-size threshold before CBC can
certify feasibility on the larger random-grid instances.
...
\begin{itemize}
  \item \emph{Heuristic coverage on the random-grid instances.}  Both
    ...
    objective values are stable across the quick benchmark run: the greedy
    heuristic reports values \num{117298}, \num{144686}, \num{164514},
    \num{242846}, and \num{196808} on the five quick instances, and the repair
    variant reproduces the same objective values on the same instances.
...
The integrated model remains significantly larger: for example, the
$|P|=10,|H|=7$ instance grows from 3\,582 to 42\,899 constraints and from
14\,540 to 14\,540 variables (variables are identical by construction), while
solve time increases from 1.1\,s to 48.6\,s under CBC.
...
\caption{Phase-3 model size and solve behaviour on the DataCplex benchmark
  (CBC, 120\,s time limit).  Vars identical across both variants (declared
  unconditionally); only the constraint count and solve behaviour differ.
  ``\emph{inf}'' = infeasible; ``\emph{err}'' = CBC time-limit abort.
  Source: \texttt{results/tables/phase3/phase3\_scalability.csv}.}
