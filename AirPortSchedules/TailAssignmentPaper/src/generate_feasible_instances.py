...
def build_feasible_instance(
    ...
    (verified below via LocalScheduler) instead of relying on the solver to
    discover a feasible plan by chance."""
    ...
    for aid in aircrafts:
        ...
        for rot in range(rotations):
            ...
            if rot == 0:
                ...
                # maintenance airport before the first return flight. For
                # C/D (calendar-day) checks, the slack must be measured
                # against the outbound flight's actual arrival time, not the
                # coarse day index: `current_day` is the departure day, but
                # the flight can arrive partway into the NEXT day once its
                # time-of-day offset and duration are added, which a fixed
...
def main() -> None:
    ...
    for index in range(args.count):
        ...
        for aid in scheduler.aircrafts:
            timeline = scheduler.get_timeline(aid, seeded_rotation.get(aid, []))
            if timeline:
