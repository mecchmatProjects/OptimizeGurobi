...
Recomputes cumulative flight-hour accumulation directly from a SOLVED
``MILP_Sheduler``'s raw ``x``/``y`` decision variables via a plain sequential
walk over each aircraft's assigned flights -- NOT via the ``mega`` hierarchy
variable and NOT by re-checking any of the model's own big-M constraint
expressions (C13/C13b). This makes it independent of bugs in those
constraints (e.g. the vacuous original Khaled et al. (2018) Eq. (13); see
docs/model_math.tex, Lemma on Constraint (13)): if the MILP reports
'optimal' but the *actual* flown schedule exceeds a check's flight-hour
threshold between two real maintenance events, this script will flag it.
...
governed by a different constraint family (C12/C12b) and are out of scope
for this script.

Usage
-----
    python src/validate_maintenance.py --data data/instances/FILE.json \\
        [--solver cbc] [--use-paper-c13] [--no-check-hierarchy] \\
        [--disable-checks B C D] [--csv results/tables/validation_report.csv]

Can also be used as a library::

    from validate_maintenance import validate_solved_model
...
sys.path.insert(0, '.')  # allow running as `python src/validate_maintenance.py` from repo root

from pyomo.environ import value as pyo_value

from model import MILP_Sheduler, run_milp
...
def _main():
    ...
    parser.add_argument('--solver', default='cbc', help='Pyomo solver name (default: cbc)')
    parser.add_argument('--time-limit', type=int, default=300)
    parser.add_argument('--use-paper-c13', action='store_true',
                        help='Solve with the original (buggy) Khaled et al. (2018) C13.')
    ...
    label = f"{args.data}  (paper_c13={args.use_paper_c13})  status={status}"
